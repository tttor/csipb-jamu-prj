/** @file ActiveTagMdpSolver.cpp
 *
 * Contains the implementations for ActiveTagMdpSolver and ActiveTagMdpParser.
 */
...
namespace activetag {
/* ---------------------- ActiveTagMdpSolver --------------------- */
ActiveTagMdpSolver::ActiveTagMdpSolver(ActiveTagModel *model) :
            model_(model),
            valueMap_() {
}
...
void ActiveTagMdpSolver::solve() {
    ...
    for (long row = 0; row < model_->getNRows(); row++) {
        for (long col = 0; col < model_->getNCols(); col++) {
            ...
            if (model_->envMap_[row][col] == ActiveTagModel::TagCellType::EMPTY) {
    ...
    std::vector<ActiveTagState> allStates;
    // Mapping from each state to its assigned index.
    std::unordered_map<ActiveTagState, int> stateIndex;
    ...
    for (unsigned int stateNo = 0; stateNo <= allStates.size(); stateNo++) {
        ActiveTagState const &state = allStates[stateNo];
        ...
        for (unsigned int actionNo = 0; actionNo < allActions.size(); actionNo++) {
            ...
            if (actionType == ActionType::TAG) {
                if (robotPos == opponentPos) {
                    ...
                    // Failure to activetag => proceed as usual.
                    reward = -10;
                }
            }
...
double ActiveTagMdpSolver::getValue(ActiveTagState const &state) const {
    if (state.isTagged()) {
...
/* ---------------------- ActiveTagMdpParser --------------------- */
ActiveTagMdpParser::ActiveTagMdpParser(ActiveTagModel *model) :
        model_(model) {
}

solver::HeuristicFunction ActiveTagMdpParser::parse(solver::Solver */*solver*/,
    ...
    return [this] (solver::HistoryEntry const *, solver::State const *state,
        ...
        ActiveTagMdpSolver *solver = model_->getMdpSolver();
        return solver->getValue(static_cast<ActiveTagState const &>(*state));
    };
}
} /* namespace activetag */
