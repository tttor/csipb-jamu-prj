/** @file ActiveTagModel.cpp
 *
 * Contains the implementations for the core functionality of the ActiveTag POMDP.
 */
...
namespace activetag {
ActiveTagUBParser::ActiveTagUBParser(ActiveTagModel *model) :
        model_(model) {
}
solver::HeuristicFunction ActiveTagUBParser::parse(solver::Solver * /*solver*/, std::vector<std::string> /*args*/) {
...
ActiveTagModel::ActiveTagModel(RandomGenerator *randGen, std::unique_ptr<ActiveTagOptions> options) :
            ModelWithProgramOptions("ActiveTag", randGen, std::move(options)),
    ...
    registerHeuristicParser("upper", std::make_unique<ActiveTagUBParser>(this));
    ...
    if (options_->hasVerboseOutput) {
        cout << "Constructed the ActiveTagModel" << endl;
...
int ActiveTagModel::getMapDistance(GridPosition p1, GridPosition p2) {
    return pairwiseDistances_[p1.i][p1.j][p2.i][p2.j];
}

void ActiveTagModel::calculateDistancesFrom(GridPosition position) {
...
void ActiveTagModel::calculatePairwiseDistances() {
...
void ActiveTagModel::initialize() {
    GridPosition p;
...
GridPosition ActiveTagModel::randomEmptyCell() {
    GridPosition pos;
    while (true) {
...
std::unique_ptr<solver::State> ActiveTagModel::sampleStateUninformed() {
    GridPosition robotPos = randomEmptyCell();
    GridPosition opponentPos = randomEmptyCell();
    return std::make_unique<ActiveTagState>(robotPos, opponentPos, false);
}

bool ActiveTagModel::isTerminal(solver::State const &state) {
    return static_cast<ActiveTagState const &>(state).isTagged();
}

bool ActiveTagModel::isValid(solver::State const &state) {
    ActiveTagState const tagState = static_cast<ActiveTagState const &>(state);
    return isValid(tagState.getOpponentPosition()) && isValid(tagState.getRobotPosition());
}


/* -------------------- Black box dynamics ---------------------- */
std::pair<std::unique_ptr<ActiveTagState>, bool> ActiveTagModel::makeNextState(
        solver::State const &state, solver::Action const &action) {
    ActiveTagState const &tagState = static_cast<ActiveTagState const &>(state);
    if (tagState.isTagged()) {
        return std::make_pair(std::make_unique<ActiveTagState>(tagState), false);
    }

    ActiveTagAction const &tagAction = static_cast<ActiveTagAction const &>(action);
    ...
    if (tagAction.getActionType() == ActionType::TAG
        ...
        return std::make_pair(
                std::make_unique<ActiveTagState>(robotPos, opponentPos, true), true);
    }
    ...
    return std::make_pair(std::make_unique<ActiveTagState>(newRobotPos, newOpponentPos, false),
            wasValid);
}

std::vector<ActionType> ActiveTagModel::makeOpponentActions(
...
std::unordered_map<GridPosition, double> ActiveTagModel::getNextOpponentPositionDistribution(
        GridPosition const &robotPos, GridPosition const &opponentPos) {
...
GridPosition ActiveTagModel::sampleNextOpponentPosition(GridPosition const &robotPos,
...
std::pair<GridPosition, bool> ActiveTagModel::getMovedPos(GridPosition const &position,
        ActionType action) {
...
bool ActiveTagModel::isValid(GridPosition const &position) {
    return (position.i >= 0 && position.i < nRows_ && position.j >= 0
            && position.j < nCols_ && envMap_[position.i][position.j] != TagCellType::WALL);
}

std::unique_ptr<solver::Observation> ActiveTagModel::makeObservation(ActiveTagState const &nextState) {
    return std::make_unique<ActiveTagObservation>(nextState.getRobotPosition(),
            nextState.getRobotPosition() == nextState.getOpponentPosition());
}

double ActiveTagModel::generateReward(solver::State const &state,
        solver::Action const &action,
        solver::TransitionParameters const * /*tp*/,
        solver::State const * /*nextState*/) {
    if (static_cast<ActiveTagAction const &>(action).getActionType()
            == ActionType::TAG) {
        ActiveTagState const &tagState = static_cast<ActiveTagState const &>(state);
...
std::unique_ptr<solver::State> ActiveTagModel::generateNextState(
        solver::State const &state, solver::Action const &action,
        solver::TransitionParameters const * /*tp*/) {
    return makeNextState(static_cast<ActiveTagState const &>(state), action).first;
}

std::unique_ptr<solver::Observation> ActiveTagModel::generateObservation(
        solver::State const * /*state*/, solver::Action const &/*action*/,
        solver::TransitionParameters const * /*tp*/,
        solver::State const &nextState) {
    return makeObservation(static_cast<ActiveTagState const &>(nextState));
}

solver::Model::StepResult ActiveTagModel::generateStep(solver::State const &state,
        solver::Action const &action) {
    solver::Model::StepResult result;
    result.action = action.copy();
    std::unique_ptr<ActiveTagState> nextState = makeNextState(state, action).first;
...
void ActiveTagModel::applyChanges(std::vector<std::unique_ptr<solver::ModelChange>> const &changes,
    ...
    for (auto const &change : changes) {
        ActiveTagChange const &tagChange = static_cast<ActiveTagChange const &>(*change);
        if (options_->hasVerboseOutput) {
...
std::vector<std::unique_ptr<solver::State>> ActiveTagModel::generateParticles(
    ...
    ActiveTagObservation const &observation =
            (static_cast<ActiveTagObservation const &>(obs));
    ActionType actionType =
            (static_cast<ActiveTagAction const &>(action).getActionType());

    typedef std::unordered_map<ActiveTagState, double> WeightMap;
    WeightMap weights;
    ...
    if (observation.seesOpponent()) {
        ...
        newParticles.push_back(
                std::make_unique<ActiveTagState>(newRobotPos, newRobotPos,
        ...
        for (solver::State const *state : previousParticles) {
            ...
            for (auto const &entry : opponentPosDistribution) {
                if (entry.first != newRobotPos) {
                    ActiveTagState newState(newRobotPos, entry.first, false);
        ...
        for (WeightMap::iterator it = weights.begin(); it != weights.end();
            ...
            for (int i = 0; i < numToAdd; i++) {
                newParticles.push_back(std::make_unique<ActiveTagState>(it->first));
            }
        }
    }
    return newParticles;
...
void ActiveTagModel::dispCell(TagCellType cellType, std::ostream &os) {
    switch (cellType) {
...
void ActiveTagModel::drawEnv(std::ostream &os) {
...
void ActiveTagModel::drawSimulationState(solver::BeliefNode const *belief,
    ...
    for (solver::State const *particle : particles) {
        GridPosition opponentPos =
                static_cast<ActiveTagState const &>(*particle).getOpponentPosition();
...
double ActiveTagModel::getDefaultHeuristicValue(solver::HistoryEntry const * /*entry*/,
...
double ActiveTagModel::getUpperBoundHeuristicValue(solver::State const &state) {
    ActiveTagState const &tagState = static_cast<ActiveTagState const &>(state);
    if (tagState.isTagged()) {
        return 0;
    }
...
std::vector<std::unique_ptr<solver::DiscretizedPoint>> ActiveTagModel::getAllActionsInOrder() {
    std::vector<std::unique_ptr<solver::DiscretizedPoint>> allActions;
    for (long code = 0; code < nActions_; code++) {
        allActions.push_back(std::make_unique<ActiveTagAction>(code));
    }
    return allActions;
}

std::vector<std::vector<float>> ActiveTagModel::getBeliefProportions(solver::BeliefNode const *belief) {
    std::vector<solver::State const *> particles = belief->getStates();
    std::vector<std::vector<long>> particleCounts(nRows_,  std::vector<long>(nCols_));
    for (solver::State const *particle : particles) {
        GridPosition targetPos = static_cast<ActiveTagState const &>(*particle).getOpponentPosition();
...
std::unique_ptr<solver::ActionPool> ActiveTagModel::createActionPool(solver::Solver */*solver*/) {
    return std::make_unique<solver::EnumeratedActionPool>(this, getAllActionsInOrder());
}
std::unique_ptr<solver::Serializer> ActiveTagModel::createSerializer(solver::Solver *solver) {
    return std::make_unique<ActiveTagTextSerializer>(solver);
}
} /* namespace activetag */
