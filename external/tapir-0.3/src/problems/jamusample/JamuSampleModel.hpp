/** @file JamuSampleModel.hpp
 *
 * Contains JamuSampleModel, which implements the core Model interface for the JamuSample POMDP.
 */
#ifndef JAMUSAMPLE_MODEL_HPP_
#define JAMUSAMPLE_MODEL_HPP_
...
/** A namespace to hold the various classes used for the JamuSample POMDP model. */
namespace jamusample {
class JamuSampleMdpSolver;
class JamuSampleState;
class JamuSampleObservation;

/** Represents a change in the JamuSample model. */
struct JamuSampleChange : public solver::ModelChange {
    /** The change type for this change - this should be one of:
     * - "Add Obstacles" - to add new obstacles to the JamuSample problem.
     * - "Remove Obstacles" - to remove existing obstacles from the JamuSample problem.
     */
...
/** The implementation of the Model interface for the JamuSample POMDP.
 *
 * See this paper ...
 * for a description of the JamuSample problem.
 *
 ...
 */
class JamuSampleModel : public shared::ModelWithProgramOptions {
friend class JamuSampleMdpSolver;
  public:
    /** Constructs a new JamuSampleModel instance with the given random number engine, and the
     * given set of configuration options.
     */
    JamuSampleModel(RandomGenerator *randGen, std::unique_ptr<JamuSampleOptions> options);
    ~JamuSampleModel() = default;
    _NO_COPY_OR_MOVE(JamuSampleModel);
    ...
    /* ----------------------- Basic getters  ------------------- */
    /** Returns the number of jamus used in this model. */
    long getNumberOfJamus() {
        return nJamus_;
    }
    ...
    void makeMdpSolver() {
        mdpSolver_ = std::make_unique<JamuSampleMdpSolver>(this);
        mdpSolver_->solve();
    }
    /** Returns the MDP solver for this model. */
    JamuSampleMdpSolver *getMdpSolver() {
    ...
    /** Returns the distance from the given grid position to the given jamu (or -1 for the goal).
     *
     * A return value of -1 means the given jamu / goal cannot be reached.
     */
    int getDistance(GridPosition p, int jamuNo) {
        std::vector<std::vector<int>> *grid = nullptr;
        if (jamuNo == -1) {
            grid = &goalDistances_;
        } else {
            grid = &jamuDistances_[jamuNo];
        }
        return (*grid)[p.i][p.j];
    }
    /** Returns true iff the given position is within the JamuSample bounds. */
    ...
    /** Returns the grid position for the given jamu. */
    GridPosition getJamuPosition(int jamuNo) {
        return jamuPositions_[jamuNo];
    }
    ...
    /** Generates particles for JamuSample using a particle filter from the previous belief.
      *
    ...
    /** Generates particles for JamuSample according to an uninformed prior.
     *
     * This method uses the fully observed part of the state, and samples the previous jamu
    ...
    /** Draws a single grid of distances to a specific jamu. */
    ...
    /** Returns all of the available actions in the JamuSample POMDP, in enumerated order. */
    ...
    /** Returns all of the possible observations in the JamuSample POMDP, in enumerated order. */
    ...
    /* ----------- Non-virtual methods for JamuSampleModel ------------- */
     ...
     */
    std::pair<std::unique_ptr<JamuSampleState>, bool> makeNextState(
            JamuSampleState const &state,
            JamuSampleAction const &action);
    /** Generates an observation given a next state (i.e. after the action)
     * and an action.
     */
    std::unique_ptr<JamuSampleObservation> makeObservation(
            JamuSampleAction const &action,
            JamuSampleState const &nextState);
    /** Retrieves the reward via the next state. */
    double makeReward(JamuSampleState const &state,
            JamuSampleAction const &action,
            JamuSampleState const &nextState,
            bool isLegal);
  ...
  private:
    /**
     * Finds and counts the jamus on the map, and initializes the required
    ...
    std::unique_ptr<JamuSampleAction> getRandomAction();

    /** Returns a random action from one of the given bins. */
    std::unique_ptr<JamuSampleAction> getRandomAction(std::vector<long> binNumbers);

    /** Generates a random position within the problem space. */
    GridPosition samplePosition();
    /** Generates the state of the jamus uniformly at random. */
    std::vector<bool> sampleJamus();
    /** Decodes jamus from an integer. */
    std::vector<bool> decodeJamus(long val);
    /** Encodes jamus to an integer. */
    long encodeJamus(std::vector<bool> jamuStates);

    /** The JamuSampleOptions instance associated with this model. */
    JamuSampleOptions *options_;

    /** The reward for sampling a good jamu. */
    double goodJamuReward_;
    /** The penalty for sampling a bad jamu. */
    double badJamuPenalty_;
    ...
    long nCols_;
    /** The number of jamus on the map. */
    long nJamus_;
    /** The starting position. */
    GridPosition startPos_;
    /** The coordinates of the jamus. */
    std::vector<GridPosition> jamuPositions_;
    ...
    /** The distance from each cell to each jamu. */
    std::vector<std::vector<std::vector<int>>> jamuDistances_;
    ...
    std::unique_ptr<JamuSampleMdpSolver> mdpSolver_;
};
} /* namespace jamusample */

#endif /* JAMUSAMPLE_MODEL_HPP_ */
