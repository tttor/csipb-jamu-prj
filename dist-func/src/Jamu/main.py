...
def calcSim(pop, m, n, p):
    ...
    for individual in pop:
        ...
        for i in range(0, n.shape[0]):
            ...
            for k in range(0, int(len(ls)*0.1)):
                if ls[k,0] == 1:
                    TP += 1
...
def getData(path, n_class, n_ref):
    ...
    for i in range(n_class):
        ...
        if i == 0:
            ...
            remaining_data = temp2[1:n, :]
        else:
            tmp1 = ref_data
            ref_data = numpy.vstack((tmp1, x[ref_idx, :]))
            
            tmp = remaining_data
            tmp2 = x[remaining_idx, :]

            if len(temp2) > 100:
                n = 100
            elif len(temp2) >= 50:
                n = 50
            elif (len(temp2) < 20) or (len(temp2) > 20):
                n = 10

            remaining_data = numpy.vstack((tmp, tmp2[1:n, :]))
