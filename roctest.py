...
def main():
    data = numpy.loadtxt(cfg.ZOO, delimiter=',')
    pair = calcpair(data)
    ...
    for idx in range(1, 6):
        ...
        CXPB, MUTPB, NGEN = cx[idx-1], mu[idx-1], 3
        ...
        numpy.savetxt(str(idx)+"-stat", logstat.values(),
                      fmt='%s',delimiter="\t")
        numpy.savetxt(str(idx)+"-pop", logpop.values(),
                      fmt='%s', delimiter="\t")
        numpy.savetxt(str(idx)+"-hof", loghof.values(),
                      fmt='%s', delimiter="\t")
