...
export class Home implements OnInit {
  ...
  constructor(public appState: AppState, private http: Http) {
    ...
    this.plaInputHolders = [{ 'index': this.nPlaInputRows, 'value' : ''}];
    this.comInputHolders = [{ 'index': this.nComInputRows, 'value' : ''}];
    this.proInputHolders = [{ 'index': this.nProInputRows, 'value' : ''}];
    this.disInputHolders = [{ 'index': this.nDisInputRows, 'value' : ''}];
  ...
  private focusPlant(index: number) {
    ...
    if (index == this.nPlaInputRows) {
      if (this.nPlaInputRows+1 < MAX_INPUT_PLANTS) {
        this.nPlaInputRows++;
        this.plaInputHolders.push({ 'index': this.nPlaInputRows, 'value' : ''});
      }
    }
  }
  ...
  private focusCompound(index: number) {
    ...
    if (index == this.nComInputRows) {
      if (this.nComInputRows+1 < MAX_INPUT_COMPOUNDS) {
        this.nComInputRows++;
        this.comInputHolders.push({ 'index': this.nComInputRows, 'value' : ''});
      }
    }
  }
  ...
  private focusProtein(index: number) {
    ...
    if (index == this.nProInputRows) {
      if (this.nProInputRows+1 < MAX_INPUT_PROTEINS) {
        this.nProInputRows++;
        this.proInputHolders.push({ 'index': this.nProInputRows, 'value' : ''});
      }
    }
  }
  ...
  private focusDisease(index: number) {
    ...
    if (index == this.nDisInputRows) {
      if (this.nDisInputRows+1 < MAX_INPUT_DISEASES) {
        this.nDisInputRows++;
        this.disInputHolders.push({ 'index': this.nDisInputRows, 'value' : ''});
      }
    }
  }
