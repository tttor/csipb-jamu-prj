...
\section{Apa Itu Ijah Webserver?}
	...
	Ada beberapa kasus-penggunaan \emph{(use-case)} dalam pencarian saat menggunakan Ijah Webserver.
		...
		\subsubsection{Use-case 2 (Drug-side inputs only)} \label{drug only}
...
\section{Tampilan Awal \emph{(Home Page)}}
	...
	\subsection{Overview}
		\subsubsection{URL} \label{URL ijah}
		...
		(1) \hyperref[URL ijah]{URL Ijah};
		(2) \hyperref[drug input]{Kotak Masukan \emph{Drug-side}};
		(3) \hyperref[target input]{Kotak Masukan \emph{Target-side}};
		(4) \hyperref[search]{Tombol Eksekusi (Search/Search and Predict)};
		(5) \hyperref[reset]{Tombol Reset};
		(6) \hyperref[example button]{Deretan Tombol Contoh};
		(7) \hyperref[menu list]{Deretan Menu};
		(8) \hyperref[judul]{Judul};
		(9) \hyperref[judul]{Logo Ijah Webserver};
		(10) \hyperref[judul]{\emph{Copyright} dan versi Ijah};
		(11) \hyperref[visitor log]{\emph{Visitor Log}}}
		\label{fig:ijah_home_overview}
		\end{figure}

	\subsection{Kotak Masukan \emph{(Input Fields)}}
		\subsubsection{Drug-side} \label{drug input}
	...
	\subsection{Judul, Versi, Logo, dan \emph{Copyright} Ijah} \label{judul}
